...
namespace AwakenServer.Farms.AElf.Tests
{
    public partial class FarmAppServiceTests
    {
        ...
        public async Task Deposit_Should_Add_User_And_Record()
        {
            ...
            userInfo.User.ShouldBe(user.ToBase58());
            ...
            var targetRecord = records.First(x =>
                ...
                DateTimeHelper.ToUnixTimeMilliseconds(depositTimestamp) / 1000 && userInfo.User == user.ToBase58() &&
        ...
        private async Task DepositAsync(Address user, string farmAddress, int pid, string txHash,
            DateTime date,
            long depositAmount)
        {
